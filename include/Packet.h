...
typedef enum
{
	...
	SERVER_JPGAMBLE_STATE,
	SERVER_JPDOOR_STATE,
	SERVER_JPSHARD_STATE,
	...
	CLIENT_JPGAMBLE_ROLL,
	CLIENT_JPSHARD_COLLECT,
	CLIENT_FART_PUSH,
	...
	// map id (u8) so waiting players can spectate the running match
	SERVER_GAME_MAPINFO,

	// no payload: a spectator asks the server to resync one-shot entity states
	CLIENT_SPECTATE_SYNC,

	// broadcast: u8 total, u8 listed, listed x string nicknames of current spectators
	SERVER_SPECTATE_LIST,

	// u8 enabled: a client enters/leaves spectator mode
	CLIENT_SPECTATE_STATE
} PacketType;
