...
bool game_init(int exe, int8_t map, Server* server)
{
	...
	server->game = (Game)
	{
		...
		.time = TICKSPERSEC,
		.elapsed = 0.0,
		.start_timeout = 10.0 * TICKSPERSEC
	};
	...
	for (size_t i = 0; i < server->peers.capacity; i++)
	{
		...
		//no one is spectating a match that is just starting
		v->spectating = false;
	...
	// Give waiting players the map id so they can spectate
	for (size_t i = 0; i < server->peers.capacity; i++)
	{
		PeerData* v = (PeerData*)server->peers.ptr[i];
		if (!v)
...
bool game_state_handletcp(PeerData* v, Packet* packet)
{
	...
	case CLIENT_SPECTATE_SYNC:
	{
		// a waiting player entered spectator mode: resend one-shot entity states
		...
		{
		case 20: // Joker's Palace
			RAssert(jp_sync(v->server, v));
			break;
		}
		break;
	}

	case CLIENT_SPECTATE_STATE:
	{
		// a client enters/leaves spectator mode, keep everyone's list up to date
		// NOTE: deliberately no in_game check, spectators are not in_game
		PacketRead(spec, packet, packet_read8, uint8_t);

		spec = spec ? true : false;
		if (v->spectating == spec)
			break;

		v->spectating = spec;
		RAssert(game_broadcast_spectators(v->server));
		break;
	}
	...
	case CLIENT_ETRACKER_ACTIVATED:
	{
		...
		// activation without a tracker (eid 0): slow mark from the demonized joker
		if (!matched)
		{
			Packet pack;
	...
	case CLIENT_CHAT_MESSAGE:
	{
		...
		{
			// clients drop packets carrying the passthrough flag, clear it before relaying
			// (same as the lobby chat relay does)
			packet->buff[0] = 0;
			RAssert(server_broadcast_ex(v->server, packet, true, v->id));
		}
...
// broadcast the current spectator nickname list to everyone
bool game_broadcast_spectators(Server* server)
{
	uint8_t total = 0;
	uint8_t listed = 0;
	...
	Packet pack;
	PacketCreate(&pack, SERVER_SPECTATE_LIST);
	PacketWrite(&pack, packet_write8, total);
	PacketWrite(&pack, packet_write8, 0); // listed count, written after the loop

	for (size_t i = 0; i < server->peers.capacity && listed < 8; i++)
	{
		PeerData* v = (PeerData*)server->peers.ptr[i];
		if (!v || !v->spectating)
			continue;
	...
	// the listed count sits in the payload byte right after the total
	pack.buff[3] = listed;

	return server_broadcast(server, &pack, true);
}
