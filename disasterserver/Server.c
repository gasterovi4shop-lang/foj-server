...
bool peer_identity_process(PeerData *v, const char *addr, bool is_banned, uint64_t timeout, bool do_timeout)
{
	...
	if (!v->in_game)
	{
		...
		if (v->op)
			server_send_msg(v->server, v->peer, CLRCODE_GRN "вам дали путЄвку в гастерович шоп" CLRCODE_RST);
...
unsigned long server_cmd_parse(String *string)
{
	...
	{
		// ѕриведение к unsigned char предотвращает передачу отрицательных значений
